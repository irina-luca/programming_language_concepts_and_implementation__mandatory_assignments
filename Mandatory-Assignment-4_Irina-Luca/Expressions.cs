...
// Abstract syntax for function definitions. 
  ...
  public class FuncDef {
    ...
    if(formArgs.Count > 0) {
      foreach(Pair<String, Type> argPair in this.formArgs) {
        if(this.formArgs.Count > 0) env.PopEnv();
      }
    }
	  if(t != returnType)  
  ...
  public class IfElseExpression : Expression {
    private readonly Expression e1, e2, e3;

    public IfElseExpression(Expression e1, Expression e2, Expression e3) {
      this.e1 = e1;
      this.e2 = e2;
      this.e3 = e3;
    }
    ...
    public override Type Check(TEnv env, FEnv fEnv) {
      ...
      if(t1 == Type.boolType) {
        ...
        if(t2 != t3) throw new TypeException("Arguments t2 and t3 must be the same type");
  ...
  public class LetExpression : Expression {
    ...
    public LetExpression(String name, Expression e1, Expression e2) {
      this.e1 = e1;
      this.e2 = e2;
      this.name = name;
    }
  ...
  public enum Operator {
    Add, Sub, Mul, Mod, Div, Neg, Eq, Ne, Lt, Le, Gt, Ge, Not, And, Or, Bad
  }
